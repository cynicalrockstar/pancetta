    ...
    public sealed partial class FlipViewPanel : UserControl, IPanel
    {
        ...
        private async Task SetPostContent(FlipViewPostItem item, bool isVisiblePost)
        {
            ...
            if (App.BaconMan.UiSettingsMan.FlipView_LoadPostContentWithoutAction)
            {
                await Task.Run(() =>
                {
                    ContentPanelMaster.Current.AddAllowedContent(ContentPanelSource.CreateFromPost(item.Context, item.Context.Post), m_uniqueId, !isVisiblePost);
                });
            }
        }
        ...
        private async void FlipViewPostPanel_OnContentLoadRequest(object sender, OnContentLoadRequestArgs e)
        {
            // Find the post
            Post post = null;
            FlipViewPostContext context = null;
            lock (m_postsLists)
            {
                foreach (FlipViewPostItem item in m_postsLists)
                {
            ...
            if (post != null)
            {
                await Task.Run(() =>
                {
                    ContentPanelMaster.Current.AddAllowedContent(ContentPanelSource.CreateFromPost(context, post), m_uniqueId, false);
                });
            }
        }
